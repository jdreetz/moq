...
export default function SupportHeader(props: SupportHeaderProps) {
	return (
		<div class="supportHeaderContainer">
			<div class="supportHeaderStatus">{getStatusText(props.summary)}</div>
			<button type="button" class="supportHeaderButton" onClick={() => props.setClosed(true)}>
				Close ❌
			</button>
			<button type="button" class="supportHeaderButton" onClick={() => props.setDetailsDisplay(!props.details)}>
